...
function getEffectiveRate(dateStr, lprData, multipliers, lprMode, lprTerm = '1y') {
  ...
  const multiplierLabel = lprMode === 'multiplier'
    ? `×${mValue}`
    : (mValue >= 0 ? `+${mValue}BP` : `${mValue}BP`);
...
export function calculate({ rateType, fixedRate, dayBase, endDate, lprData, multipliers, lprMode, lprTerm = '1y', loans, repays }) {
  ...
  validLoans.forEach((l) =>
    events.push({ date: l.date, amount: l.amount, type: 'loan', note: l.note || '' })
  );
      ...
      if (ev.date === curDate) {
        if (ev.type === 'loan') {
          ...
          if (ev.mode === '先息后本') {
            ...
            if (interestPaid > 0) {
              rows.push({ ...makeEventRow(curDate, '还款(冲息)', interestPaid, principal + principalPaid, firstNote), interest: (-interestPaid).toFixed(2) });
            }
            if (principalPaid > 0) {
              rows.push(makeEventRow(curDate, '还款(还本)', principalPaid, principal, isSplit && interestPaid > 0 ? '' : (ev.note || ev.mode)));
            }
            ...
            if (interestPaid > 0) {
              rows.push({ ...makeEventRow(curDate, '还款(冲息)', interestPaid, principal, isSplit && principalPaid > 0 ? '' : (ev.note || ev.mode)), interest: (-interestPaid).toFixed(2) });
            }
